...
import { LuSparkles, LuMessageCircle, LuChartLine, LuArrowDown, LuArrowUp, LuChevronDown, LuGithub } from "react-icons/lu";
...
export default function Home() {
  ...
  // Redirect logged-in users to their first workspace
  useEffect(() => {
    if (!ready || !user?.handle || redirecting) return;
    setRedirecting(true);
    const API_BASE = process.env.NEXT_PUBLIC_HIVE_SERVER ?? "/api";
    fetch(`${API_BASE}/workspaces`, { headers: getAuthHeader() })
      .then((r) => r.json())
      .then((data) => {
        const workspaces = data?.workspaces ?? [];
        if (workspaces.length > 0) {
          router.replace(`/${user.handle}/${workspaces[0].name}`);
        } else {
  ...
  }, [ready, user, router, redirecting]);

  // Show landing page for non-logged-in users
  if (ready && user?.handle) return null;
  if (!ready) {
    return (
