...
export function AuthModal({ onClose, initialMode = "login" }: AuthModalProps) {
  ...
  const title = mode === "verify" ? "Verify your email" : mode === "forgot" ? "Forgot password" : mode === "reset" ? "Reset password" : mode === "login" ? "Log in" : "Sign up";

  return (
    <Modal onClose={onClose} maxWidth="max-w-[380px]">
