...
export function TaskTerminalModal({ taskId, open, onClose }: TaskTerminalModalProps) {
  const { user } = useAuth();
  ...
  const loadSandbox = useCallback(async () => {
    ...
    try {
      const data = await apiFetch<SandboxInfo>(`/tasks/${taskId}/sandbox`);
      setSandbox(data);
    } catch (e) {
      setSandbox(null);
  ...
  const createSandbox = async () => {
    ...
    setSandboxError(null);
    try {
      const res = await fetch(`${API_BASE}/tasks/${encodeURIComponent(taskId)}/sandbox`, {
        method: "POST",
      ...
      if (data.status === "creating") {
        const t = setInterval(async () => {
          try {
            const s = await apiFetch<SandboxInfo>(`/tasks/${taskId}/sandbox`);
            setSandbox(s);
  ...
  const deleteSandbox = async () => {
    ...
    try {
      await apiDelete(`/tasks/${taskId}/sandbox`, getAuthHeader());
    } catch (e) {
  ...
  const reconnectSession = async (session: SandboxTerminalSessionRow) => {
    ...
    try {
      const data = await apiPostJson<{ ticket: string }>(
        `/tasks/${taskId}/sandbox/sessions/${session.id}/ticket`,
        {},
        getAuthHeader(),
      );
  ...
  const closeSession = async (sessionId: number) => {
    ...
    try {
      await apiDelete(`/tasks/${taskId}/sandbox/sessions/${sessionId}`, getAuthHeader());
      await loadSessions();
    } catch {
      /* ignore */
    }
