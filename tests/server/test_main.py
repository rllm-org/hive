...
class TestAgentTokens:
    ...
    def test_agent_auth_with_uuid_token(self, client, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"token": agent_token})
        assert resp.status_code == 200
...
def _insert_task(slug="t1", name="Test Task", description="A test", config=None,
                 owner="hive"):
...
class TestCreateTask:
    def test_create(self, client, monkeypatch):
        ...
        assert resp.status_code == 201
        assert resp.json()["slug"] == "gsm8k"
        assert isinstance(resp.json()["id"], int)
        assert resp.json()["owner"] == "hive"
        assert resp.json()["repo_url"] == "https://github.com/hive/task--gsm8k"
        assert resp.json()["status"] == "active"

    def test_description_too_long(self, client, monkeypatch):
        resp = _post_task(client, slug="too-long", description="x" * 351, headers=_admin_headers(monkeypatch))
        assert resp.status_code == 400
        assert "350" in resp.json()["detail"]

    def test_duplicate_task(self, client, monkeypatch):
        headers = _admin_headers(monkeypatch)
        _post_task(client, slug="t1", name="T", description="D", headers=headers)
        resp = _post_task(client, slug="t1", name="T", description="D", headers=headers)
        assert resp.status_code == 409
    ...
    def test_missing_fields(self, client):
        ...
        assert client.post("/api/tasks", data={"slug": "x", "name": "X"},
...
class TestGetTask:
    def test_not_found(self, client):
        resp = client.get("/api/tasks/hive/nope")
        assert resp.status_code == 404
...
class TestSubmitRun:
    ...
    def test_submit_no_sha(self, registered_agent, _seed_task):
        client, _, token = registered_agent
        resp = client.post(
            "/api/tasks/hive/t1/submit", params={"token": token}, json={"message": "hi"}
        )
        assert resp.status_code == 400
    ...
    def test_submit_auto_fills_fork_id(self, registered_agent, _seed_task, mock_github):
        client, _, token = registered_agent
        client.post("/api/tasks/hive/t1/clone", params={"token": token})
    ...
    def test_submit_verifiable_run_sets_pending_without_updating_task_stats(self, registered_agent, mock_github):
        ...
        clone = client.post("/api/tasks/hive/tv2/clone", params={"token": token})
        assert clone.status_code == 201

        resp = client.post(
            "/api/tasks/hive/tv2/submit",
            params={"token": token},
        ...
        task = client.get("/api/tasks/hive/tv2").json()
    ...
    def test_submit_verifiable_run_queues_even_without_reported_score(self, registered_agent, mock_github):
        ...
        clone = client.post("/api/tasks/hive/tv3/clone", params={"token": token})
        assert clone.status_code == 201

        resp = client.post(
            "/api/tasks/hive/tv3/submit",
            params={"token": token},
...
class TestListRuns:
    def test_best_runs(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs")
        assert resp.status_code == 200
        data = resp.json()
        runs = data["runs"]
    ...
    def test_contributors_view(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"view": "contributors"})
        data = resp.json()
    ...
    def test_deltas_view(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"view": "deltas"})
        data = resp.json()
    ...
    def test_improvers_view(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"view": "improvers"})
        data = resp.json()
    ...
    def test_sort_score_asc(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"sort": "score:asc"})
        runs = resp.json()["runs"]
        assert runs[0]["score"] <= runs[-1]["score"]
    ...
    def test_sort_score_desc_explicit(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"sort": "score:desc"})
        runs = resp.json()["runs"]
        assert runs[0]["score"] >= runs[-1]["score"]
    ...
    def test_sort_invalid_direction_defaults_desc(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"sort": "score:invalid"})
        runs = resp.json()["runs"]
        assert runs[0]["score"] >= runs[-1]["score"]

    def test_task_not_found(self, client):
        resp = client.get("/api/tasks/hive/nope/runs")
        assert resp.status_code == 404
    ...
    def test_verified_only_uses_verified_score_even_without_reported_score(self, registered_agent, mock_github):
        ...
        client.post("/api/tasks/hive/tv4/clone", params={"token": token})
        client.post(
            "/api/tasks/hive/tv4/submit",
            params={"token": token},
        ...
        resp = client.get("/api/tasks/hive/tv4/runs", params={"verified_only": True})
        assert resp.status_code == 200
        runs = resp.json()["runs"]
...
class TestGetRun:
    def test_get(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs/r1")
        assert resp.status_code == 200
        assert resp.json()["id"] == "r1"

    def test_not_found(self, client):
        resp = client.get("/api/tasks/hive/t1/runs/nope")
        assert resp.status_code == 404
    ...
    def test_get_run_includes_fork_url(self, registered_agent, _seed_task, mock_github):
        ...
        resp = client.get("/api/tasks/hive/t1/runs/forksha1")
        assert resp.status_code == 200
...
class TestPatchRun:
    def test_invalidating_verified_run_recomputes_task_stats(self, registered_agent, monkeypatch, mock_github):
        ...
        client.post("/api/tasks/hive/tv-patch/clone", params={"token": token})
        client.post(
            "/api/tasks/hive/tv-patch/submit",
            params={"token": token},
        ...
        with get_db_sync() as conn:
            ...
            conn.execute(
                "UPDATE tasks SET best_score = 0.9, improvements = 2 WHERE owner = %s AND slug = %s",
                ("hive", "tv-patch"),
            )

        resp = client.patch(
            "/api/tasks/hive/tv-patch/runs/patchhigh1",
            headers=headers,
            json={"valid": False},
        )
        assert resp.status_code == 200
        ...
        task = client.get("/api/tasks/hive/tv-patch").json()
        assert task["stats"]["best_score"] == 0.4
        assert task["stats"]["improvements"] == 1

        verified_runs = client.get("/api/tasks/hive/tv-patch/runs", params={"verified_only": True}).json()["runs"]
...
class TestDeleteRun:
    def test_delete_single_run(self, registered_agent, _seed_task, monkeypatch):
        ...
        resp = client.delete("/api/tasks/hive/t1/runs/del1", headers=headers)
        assert resp.status_code == 200
    ...
    def test_delete_run_updates_best_score(self, registered_agent, _seed_task, monkeypatch):
        ...
        client.delete("/api/tasks/hive/t1/runs/hi1", headers=headers)
        task = client.get("/api/tasks/hive/t1").json()
        assert task["stats"]["best_score"] == 0.3

    def test_delete_nonexistent_run(self, registered_agent, _seed_task, monkeypatch):
        client, _, token = registered_agent
        resp = client.delete("/api/tasks/hive/t1/runs/nope", headers=_admin_headers(monkeypatch))
        assert resp.status_code == 404
    ...
    def test_delete_all_runs(self, registered_agent, _seed_task, monkeypatch):
        ...
        resp = client.delete("/api/tasks/hive/t1/runs", headers=headers)
        assert resp.status_code == 200
        assert resp.json()["deleted"] == 3
        # Runs should be empty
        runs_resp = client.get("/api/tasks/hive/t1/runs")
    ...
    def test_delete_all_runs_task_not_found(self, registered_agent, monkeypatch):
        client, _, token = registered_agent
        resp = client.delete("/api/tasks/hive/nope/runs", headers=_admin_headers(monkeypatch))
        assert resp.status_code == 404
    ...
    def test_delete_verified_run_recomputes_official_stats(self, registered_agent, monkeypatch, mock_github):
        ...
        client.post("/api/tasks/hive/tv-delete/clone", params={"token": token})
        client.post(
            "/api/tasks/hive/tv-delete/submit",
            params={"token": token},
        ...
        resp = client.delete("/api/tasks/hive/tv-delete/runs/dehigh1", headers=headers)
        assert resp.status_code == 200

        task = client.get("/api/tasks/hive/tv-delete").json()
        assert task["stats"]["best_score"] == 0.4
        assert task["stats"]["improvements"] == 1
...
class TestDeleteTask:
    ...
    def test_delete_empty_task(self, client, _seed_task):
        ...
        assert resp.status_code == 200
        assert isinstance(resp.json()["deleted_task"], int)
    ...
    def test_delete_task_not_found(self, client):
        resp = client.delete("/api/tasks/hive/nope?confirm=nope", headers=self._admin)
        assert resp.status_code == 404

    def test_delete_task_confirm_mismatch(self, client, _seed_task):
        resp = client.delete("/api/tasks/hive/t1?confirm=wrong", headers=self._admin)
        assert resp.status_code == 400
        assert client.get("/api/tasks/hive/t1").status_code == 200

    def test_delete_task_missing_confirm(self, client, _seed_task):
        resp = client.delete("/api/tasks/hive/t1", headers=self._admin)
        assert resp.status_code == 422

    def test_delete_task_requires_admin(self, client, _seed_task):
        resp = client.delete("/api/tasks/hive/t1?confirm=t1", headers={"X-Admin-Key": "wrong"})
        assert resp.status_code == 403
...
class TestContext:
    ...
    def test_verifiable_context_leaderboard_uses_verified_scores(self, registered_agent, mock_github):
        ...
        client.post("/api/tasks/hive/tv5/clone", params={"token": token})
        client.post(
            "/api/tasks/hive/tv5/submit",
            params={"token": token},
        ...
        with get_db_sync() as conn:
            ...
            conn.execute(
                "UPDATE tasks SET best_score = 0.7, improvements = 1 WHERE owner = %s AND slug = %s",
                ("hive", "tv5"),
            )

        resp = client.get("/api/tasks/hive/tv5/context")
        assert resp.status_code == 200
...
class TestCloneTask:
    def test_clone_creates_copy(self, registered_agent, _seed_task, mock_github):
        client, agent_id, token = registered_agent
        resp = client.post("/api/tasks/hive/t1/clone", params={"token": token})
        assert resp.status_code == 201
        data = resp.json()
    ...
    def test_clone_idempotent(self, registered_agent, _seed_task, mock_github):
        client, _, token = registered_agent
        resp1 = client.post("/api/tasks/hive/t1/clone", params={"token": token})
        resp2 = client.post("/api/tasks/hive/t1/clone", params={"token": token})
    ...
    def test_clone_bad_token(self, client, _seed_task):
        resp = client.post("/api/tasks/hive/t1/clone", params={"token": "fake"})
        assert resp.status_code == 401

    def test_clone_task_not_found(self, registered_agent):
        client, _, token = registered_agent
        resp = client.post("/api/tasks/hive/nope/clone", params={"token": token})
        assert resp.status_code == 404
...
class TestGraph:
    def test_empty_graph(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/graph")
        assert resp.status_code == 200
        data = resp.json()
        assert data["nodes"] == []
    ...
    def test_graph_task_not_found(self, client):
        resp = client.get("/api/tasks/hive/nope/graph")
        assert resp.status_code == 404
        ...
        with get_db_sync() as conn:
            for tid in ("ta", "tb"):
                conn.execute(
                    ...
                    (tid, "hive", tid, "desc", "https://github.com/test/test", now()),
                )
        # Submit to both tasks
        client.post("/api/tasks/hive/ta/submit", params={"token": token},
                    json={"sha": "sha-a", "message": "m", "score": 0.5})
        client.post("/api/tasks/hive/tb/submit", params={"token": token},
...
class TestLimitParamRemoved:
    ...
    def test_runs_uses_per_page(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1/runs", params={"per_page": 2})
...
class TestImprovementsDenormalization:
    ...
    def test_new_best_increments(self, registered_agent, _seed_task):
        ...
        resp = client.get("/api/tasks/hive/t1")
