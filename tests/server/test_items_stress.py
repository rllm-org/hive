...
def _post_task(client, slug="stress-task"):
    with psycopg.connect(_db.DATABASE_URL, autocommit=True) as conn:
        conn.execute(
            ...
            (slug, slug, "stress test", "https://github.com/test", _db.now()),
        )


def _post_task_no_seq(client, slug="no-seq-task"):
    ...
    with psycopg.connect(_db.DATABASE_URL, autocommit=True) as conn:
        conn.execute(
            ...
            (slug, slug, "no seq", "https://github.com/test", _db.now()),
        )
...
class TestTitleBoundary:
    def test_title_500_chars_passes(self, client):
        ...
        resp = client.post(
            "/api/tasks/hive/stress-task/items",
            json={"title": "x" * 500},
            params={"token": token},
        )
...
class TestCommentBoundary:
    def test_comment_5000_chars_passes(self, client):
        _post_task(client)
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        resp = client.post(
            "/api/tasks/hive/stress-task/items/STRESS-1/comments",
            json={"content": "x" * 5000},
            params={"token": token},
        )
        assert resp.status_code == 201
...
class TestEdgeCases:
    def test_create_item_on_task_without_item_seq(self, client):
        ...
        token = _register(client)
        resp = client.post(
            "/api/tasks/hive/no-seq-task/items",
    ...
    def test_patch_empty_body_fails(self, client):
        ...
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        resp = client.patch(
            "/api/tasks/hive/stress-task/items/STRESS-1",
            json={},
            params={"token": token},
        )
        assert resp.status_code == 400
    ...
    def test_delete_already_soft_deleted_item_404(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token})
        resp = client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token})
    ...
    def test_get_soft_deleted_item_404(self, client):
        ...
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token})
        resp = client.get("/api/tasks/hive/stress-task/items/STRESS-1")
        assert resp.status_code == 404
    ...
    def test_assign_same_agent_idempotent(self, client):
        ...
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        r1 = client.post("/api/tasks/hive/stress-task/items/STRESS-1/assign", params={"token": token})
        assert r1.status_code == 200
        r2 = client.post("/api/tasks/hive/stress-task/items/STRESS-1/assign", params={"token": token})
    ...
    def test_create_item_with_deleted_parent_fails(self, client):
        _post_task(client)
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "parent"}, params={"token": token})
        client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token})
        resp = client.post(
            "/api/tasks/hive/stress-task/items",
    ...
    def test_negation_filter_nonexistent_status(self, client):
        ...
        _post_task(client)
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item 1", "status": "backlog"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item 2", "status": "archived"}, params={"token": token})
        resp = client.get("/api/tasks/hive/stress-task/items", params={"status": "!nonexistent"})
        assert resp.status_code == 400
...
class TestAuthorization:
    def test_agent_b_cannot_delete_agent_a_item(self, client):
        ...
        client.post("/api/tasks/hive/stress-task/items", json={"title": "A's item"}, params={"token": token_a})
        resp = client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token_b})
    ...
    def test_agent_b_cannot_delete_agent_a_comment(self, client):
        ...
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token_a})
        ...
        resp = client.delete(
            f"/api/tasks/hive/stress-task/items/STRESS-1/comments/{comment_id}",
    ...
    def test_missing_token_on_create_returns_401(self, client):
        _post_task(client)
        resp = client.post("/api/tasks/hive/stress-task/items", json={"title": "item"})
        assert resp.status_code == 401
    ...
    def test_missing_token_on_patch_returns_401(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        resp = client.patch("/api/tasks/hive/stress-task/items/STRESS-1", json={"status": "archived"})
        assert resp.status_code == 401
    ...
    def test_missing_token_on_delete_returns_401(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        resp = client.delete("/api/tasks/hive/stress-task/items/STRESS-1")
        assert resp.status_code == 401
    ...
    def test_missing_token_on_assign_returns_401(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "item"}, params={"token": token})
        resp = client.post("/api/tasks/hive/stress-task/items/STRESS-1/assign")
        assert resp.status_code == 401
...
class TestDataIntegrity:
    def test_parent_child_grandchild_delete_chain(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Parent"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Child", "parent_id": "STRESS-1"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Grandchild", "parent_id": "STRESS-2"}, params={"token": token})

        # Cannot delete child while grandchild exists
        r = client.delete("/api/tasks/hive/stress-task/items/STRESS-2", params={"token": token})
        assert r.status_code == 409

        # Delete grandchild first
        r = client.delete("/api/tasks/hive/stress-task/items/STRESS-3", params={"token": token})
        ...
        # Now delete parent
        r = client.delete("/api/tasks/hive/stress-task/items/STRESS-1", params={"token": token})
        assert r.status_code == 204
    ...
    def test_comment_count_accurate_after_create_and_delete(self, client):
        ...
        item_resp = client.get("/api/tasks/hive/stress-task/items/STRESS-1")
        assert item_resp.json()["comment_count"] == 3

        # Delete one comment
        client.delete(f"/api/tasks/hive/stress-task/items/STRESS-1/comments/{comment_ids[0]}", params={"token": token})
    ...
    def test_children_list_excludes_soft_deleted_children(self, client):
        ...
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Parent"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Child 1", "parent_id": "STRESS-1"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Child 2", "parent_id": "STRESS-1"}, params={"token": token})

        # Soft-delete child 1 (no grandchildren so delete allowed)
        client.delete("/api/tasks/hive/stress-task/items/STRESS-2", params={"token": token})
    ...
    def test_list_items_excludes_soft_deleted(self, client):
        _post_task(client)
        token = _register(client)
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Keep"}, params={"token": token})
        client.post("/api/tasks/hive/stress-task/items", json={"title": "Delete me"}, params={"token": token})
        client.delete("/api/tasks/hive/stress-task/items/STRESS-2", params={"token": token})

        resp = client.get("/api/tasks/hive/stress-task/items")
        assert resp.status_code == 200
    ...
    def test_comment_count_zero_after_all_comments_deleted(self, client):
        ...
        client.delete(f"/api/tasks/hive/stress-task/items/STRESS-1/comments/{comment_id}", params={"token": token})
    ...
    def test_comment_count_in_list_view_matches_get_view(self, client):
        ...
        get_resp = client.get("/api/tasks/hive/stress-task/items/STRESS-1")
        list_resp = client.get("/api/tasks/hive/stress-task/items")
